	...
	private String namePrepieceRev( boolean debug, TableToArrayListMultimapHikari almmPrepiece, String name, int id ) throws Exception
	{
		...
		while( ! names.isEmpty() )		// Check pieces
		{
			...
			if( almmPrepiece.contains( part ) && almmPrepiece.code( part ).equalsIgnoreCase( SC_Y ) )
			{
				...
				{
					list_TO = almmPrepiece.value( "title_other", part ) + " " + list_TO;
				}
				else if( almmPrepiece.value("prefix", part) != null && !almmPrepiece.value( "prefix", part ).isEmpty() )
				{
...
/**
	...
	private void doPostTasks( boolean debug, boolean go, String source, String rmtype ) throws Exception
	{
		...
		showMessage( msg, false, true );
		updateSexRole( debug, source, rmtype );
	 ...
	 * @throws Exception
	 */
	private void updateSexRole( boolean debug, String source, String rmtype )
		throws Exception
	{
		long threadId = Thread.currentThread().getId();

		if( debug ) { showMessage( "updateSexRole()", false, true ); }
	...
	private void doFlagPersonRecs( boolean debug, boolean go, String source, String rmtype ) throws Exception
	{
		...
		msg = String.format( "Thread id %02d; Flagging Person recs with invalid role for source: %s, rmtype: %s ...", threadId, source, rmtype );
		showMessage( msg, false, true );
		...
		throws Exception
	{
		...
		String query = "SELECT id_person, role, not_linksbase_p FROM person_c"
			+ " WHERE id_source = " + source
		...
		try( PreparedStatement pstmt = dbconCleaned.prepareStatement( query ) )
		{
			try( ResultSet rs = pstmt.executeQuery() )
			{
				while( rs.next() )        // process all persons
				{
					int id_person    = rs.getInt( "id_person" );
					int role         = rs.getInt( "role" );
					String old_flags = rs.getString( "not_linksbase_p" );
					...
					if( ! new_flags.isEmpty() )
					{
						String flagQuery = "UPDATE person_c SET not_linksbase_p = '" + new_flags + "'";
						flagQuery += " WHERE id_person = " + id_person + ";";
						if( debug ) { System.out.println( flagQuery ); }

						countRegist = dbconCleaned.executeUpdate( flagQuery );
						nNoRole += countRegist;
					}
				}

				String msg = String.format( "Thread id %02d; Number of person records with invalid role flagged: %d", threadId, nNoRole );
				showMessage( msg, false, true );
			}
		}
		catch( Exception ex ) {
		...
		throws Exception
	{
		...
		String query = "SELECT id_person, not_linksbase_p FROM person_c"
			+ " WHERE id_source = " + source
		...
		throws Exception
	{
		...
		try( PreparedStatement pstmt = dbconCleaned.prepareStatement( query ) )
		{
			try( ResultSet rs = pstmt.executeQuery() )
			{
				...
				String msg = String.format( "Thread id %02d; Number of person records with missing firstname flagged: %d", threadId, nNoFirstName );
				showMessage( msg, false, true );
			}
		}
		catch( Exception ex ) {
	...
	private void scanRemarks( boolean debug, String source, String rmtype ) throws Exception
	{
		...
		try( ResultSet rs_r = dbconRef.executeQuery( selectQuery_r ) )
		{
			...
			while( rs_r.next() )
			{
				...
				String not_string_1 = rs_r.getString( "not_string_1" );
				String not_string_2 = rs_r.getString( "not_string_2" );
				...
				ref_remarks.setNotString_1( not_string_1 );
				ref_remarks.setNotString_2( not_string_2 );
				...
				if( debug ) { System.out.printf( "%2d, id_scan: %3d, maintype: %d, role: %2d, string_1: %s, string_2: %s, string_3: %s, not_string_1: %s, not_string_2: %s,  name_table: %s, name_field: %s, value: %s\n",
					nrecord, id_scan, maintype, role, string_1, string_2, string_3, not_string_1, not_string_2, name_table, name_field, value ); }
		...
		try( ResultSet rs_o = dbconOriginal.executeQuery( selectQuery_o ) )
		{
			...
			while( rs_o.next() )
			{
				...
				for( Remarks ref_remarks : ref_remarksVec )
				{
					...
					String not_string_1 = ref_remarks.getNotString_1();
					String not_string_2 = ref_remarks.getNotString_2();
					...
					if( found )
					{
						...
						else    // not_string_1 and not_string_2 not both empty
						{
							if( remarks_str.indexOf( not_string_1 ) == -1 && remarks_str.indexOf( not_string_2 ) == -1  )   // but neither found
